...
class MCRDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        input_raw = imageio.v2.imread(os.path.join(self.data_dir,input_path)).transpose(2,0,1)#(1,h,w) numpy
        gt_raw = imageio.v2.imread(os.path.join(self.data_dir,gt_raw_path)).transpose(2,0,1)#(1,h,w) numpy
        gt_rgb = imageio.v2.imread(os.path.join(self.data_dir,gt_rgb_path)).transpose(2,0,1)#(3,1024,1280) numpy
...
if __name__=='__main__':
    dataset = MCRDataset(data_dir='/data/dataset/Carlos/MCR/',
