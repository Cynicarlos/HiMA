...
class SIDFujiDataset(data.Dataset):
    def __init__(self, data_dir, image_list_file, patch_size=None, split='train', 
                 load_npy=False, with_metatada=False, metarange_file='metarange.txt',  transpose=False, h_flip=False, v_flip=False, ratio=False, **kwargs):
        assert os.path.exists(data_dir), "data_path: {} not found.".format(data_dir)
        self.data_dir = data_dir
        image_list_file = os.path.join(data_dir, image_list_file)
        assert os.path.exists(image_list_file), "image_list_file: {} not found.".format(image_list_file)
    ...
    def __getitem__(self, index):
        info = self.img_info[index]
        input_path = info['input_path'] #/Fuji/short/10019_05_0.033s.RAF
        gt_path = info['gt_path']
        if self.load_npy:
            input_raw = np.load(os.path.join(self.data_dir, 'Fuji/npy_short/', os.path.basename(input_path)[:-4]+'.npy'))
            gt_raw = np.load(os.path.join(self.data_dir, 'Fuji/npy_long/', os.path.basename(gt_path)[:-4]+'.npy'))
            gt_rgb = np.load(os.path.join(self.data_dir, 'Fuji/npy_long/', os.path.basename(gt_path)[:-4]+'_rgb.npy'))
        else:
            input_raw = rawpy.imread(os.path.join(self.data_dir, input_path))
            input_raw = self.pack_raw(input_raw)#(9,4032//3,6032//3) numpy
            gt_raw = rawpy.imread(os.path.join(self.data_dir, gt_path))
            gt_rgb = gt_raw.postprocess(use_camera_wb=True, half_size=False, no_auto_bright=True, output_bps=16)
            gt_rgb = gt_rgb.transpose(2, 0, 1)#未归一 (9,4032,6032) numpy
            gt_raw = self.pack_raw(gt_raw)#(9,4032//3,6032//3) numpy
            packed_H, packed_W = gt_raw.shape[1:]
            #to ensure the shape of the output of the model is same as gt_rgb
            gt_rgb = gt_rgb[:, :3*packed_H, :3*packed_W]#(9,4032//3*3,6032//3*3) numpy
