...
class MultiPriorFuse(nn.Module):
    def __init__(self, dims, in_channels):
        super().__init__()
        self.refien = nn.Conv2d(dims, dims, 1)
    def forward(self, x, y):
        x = self.refine(x) + y
        return x
...
if __name__ == '__main__':
    ...
    model = HiMA_MPF_X_Y(in_channels=4, dims=32, layers=4, num_blocks_per_layer=[2,2,2,2]).to(device='cuda') 
