...
class SimpleFuse(nn.Module):
    def __init__(self, dims, in_channels):
        super().__init__()
        self.body = nn.Conv2d(2*dims+2*in_channels, dims, 1)
    def forward(self, x, y, z, k):
        x = self.body(torch.cat([x,y,z,k],dim=1))
        return x
...
class Baseline_MeSA_LoDA(nn.Module):
    def __init__(self, in_channels=3, dims=32, layers=4, num_blocks_per_layer=[1,1,1,1],with_local_mean_std_sup=False):
        ...
        self.input_local_distribution_align = LocalDistributionAlign(in_channels=in_channels, dims=dims)
        ...
        self.fuses = nn.ModuleList([
            SimpleFuse(dims=dims*2**(i), in_channels=in_channels) for i in range(layers-2, -1, -1)
        ])
...
if __name__ == '__main__':
    ...
    model = Baseline_MeSA_LoDA(in_channels=4, dims=32, layers=4, num_blocks_per_layer=[2,2,2,2]).to(device='cuda') 
