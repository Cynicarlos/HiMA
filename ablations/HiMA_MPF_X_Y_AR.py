...
class MultiPriorFuse(nn.Module):
    ...
    def forward(self, x, y, aligned_raw):
        '''
        x,y (b,c,h,w)
        prior (b, c_in, h, w)
        '''
        aligned_raw = self.refine_aligned_raw(aligned_raw)
        y = self.refine1(y + aligned_raw)
        x = self.refine2(x)
        x = x + y
        return x
...
class HiMA_MPF_X_Y_AR(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.ups[0](x)
        x = self.fuses[0](x, encoder_features[0], aligned_raws[0])
        for decoder in self.decoder3:
            x = decoder(x)

        x = self.ups[1](x)
        x = self.fuses[1](x, encoder_features[1], aligned_raws[1])
        for decoder in self.decoder2:
            x = decoder(x, self.learnable_metadata)

        x = self.ups[2](x)
        x = self.fuses[2](x, encoder_features[2], aligned_raws[2])
...
if __name__ == '__main__':
    ...
    model = HiMA_MPF_X_Y_AR(in_channels=4, dims=32, layers=4, num_blocks_per_layer=[2,2,2,2]).to(device='cuda') 
